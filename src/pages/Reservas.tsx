...
type Firmo = 'all' | 'PENDIENTE' | 'FIRMADO'
...
const UNIDADES_BY_TAB: Record<string, string[]> = {
  ...
  RESIDENCIAL: ['PLAT. RECOLETA','PLAT. PALERMO','PLAT. BELGRANO','PLAT. CABALLITO','PLAT. FLORES','PLAT. NORDELTA','DPTO DE BÚSQUEDA'],
  COMERCIAL: ['LOCALES Y TERRENOS','OFICINAS Y EDIFICIOS','CONSULTORIA'],
}
...
export default function Reservas(_: any) {
  ...
  const [firmoFilt, setFirmoFilt] = useState<Firmo>('all')
  ...
  const load = async () => {
    setLoading(true)
    const { data } = await supabase.from('reservas').select('*').order('fecha', { ascending: false })
    setAll(data || [])
    setLoading(false)
  }
  useEffect(() => { load() }, [])
  ...
  // Filter logic
  const getTabData = (t: Tab) => {
    if (t === 'DASHBOARD') return all
    const units = UNIDADES_BY_TAB[t] || []
    let rows = all.filter(r => {
      if (t === 'EMPRENDIMIENTOS') return r.unidad === 'EMPRENDIMIENTOS'
      if (t === 'RESIDENCIAL')    return !UNIDADES_BY_TAB.EMPRENDIMIENTOS.includes(r.unidad) && !UNIDADES_BY_TAB.COMERCIAL.includes(r.unidad) && r.unidad !== 'EMPRENDIMIENTOS'
      if (t === 'COMERCIAL')      return UNIDADES_BY_TAB.COMERCIAL.includes(r.unidad)
      return true
    })
  ...
  all.forEach(r => { byBroker[r.broker || 'Sin broker'] = (byBroker[r.broker || 'Sin broker'] || 0) + 1 })
  const topBrokers = Object.entries(byBroker).sort((a, b) => b[1] - a[1]).slice(0, 6)

  // By mes
  const byMes = new Array(12).fill(0)
  all.forEach(r => {
    const m = parseInt(r.fecha?.slice(5, 7) || '0') - 1
    if (m >= 0 && m < 12) byMes[m]++
  })
      ...
      barChart.current = new Chart(barRef.current, {
        ...
        data: {
          ...
          datasets: [
            {
              label: 'Ventas',
              data: new Array(12).fill(0).map((_, i) => all.filter(r => r.operacion === 'VENTA' && parseInt(r.fecha?.slice(5,7)||'0')-1 === i).length),
              backgroundColor: 'rgba(200,16,46,0.7)',
              borderRadius: 3,
            },
            {
              label: 'Alquileres',
              data: new Array(12).fill(0).map((_, i) => all.filter(r => r.operacion === 'ALQUILER' && parseInt(r.fecha?.slice(5,7)||'0')-1 === i).length),
        ...
        options: {
          ...
          scales: {
            ...
            y: { grid: { color: 'rgba(255,255,255,0.06)' }, ticks: { color: '#a8a8a8', font: { size: 10 } } }
          },
  ...
  return (
    <div style={{ '--accent': '#1a6bc8', '--accent-hover': '#1558a8', '--accent-bg': 'rgba(26,107,200,0.12)', '--accent-text': '#6eb3ff', '--border-red': 'rgba(26,107,200,0.4)' } as any}>
      ...
      <div style={{ marginBottom: 18, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: 'var(--text-tertiary)' }}>{all.length} reservas registradas · {all.filter(r=>r.firmo==='FIRMADO').length} firmadas</p>
        </div>
        {tab !== 'DASHBOARD' && (
          ...
          {loading ? <Spinner /> : (
            <>
              <div className="metrics-grid">
                <div className="metric-card" style={{ borderColor: 'rgba(26,107,200,0.4)' }}>
              ...
              <div className="two-col">
                ...
                <div className="card">
                  <div className="card-header"><span className="card-title">Top brokers por cantidad</span></div>
                  <div style={{ padding: '12px 16px' }}>
                    {topBrokers.map(([broker, count]) => (
                      <div key={broker} className="progress-row" style={{ marginBottom: 8 }}>
                        ...
                        <span style={{ fontSize: 11, color: 'var(--text-secondary)', minWidth: 40, textAlign: 'right' }}>{count}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="two-col">
                {(['EMPRENDIMIENTOS','RESIDENCIAL','COMERCIAL'] as Tab[]).map(t => {
                  ...
                  const ventas = d.filter(r=>r.operacion==='VENTA').length
                  const alq    = d.filter(r=>r.operacion==='ALQUILER').length
                  ...
                  return (
                    <div key={t} className="card" style={{ cursor: 'pointer' }} onClick={() => setTab(t)}>
                      ...
                      <div style={{ padding: '12px 16px' }}>
                        <div className="sum-row"><span>Ventas</span><span style={{ fontWeight: 500 }}>{ventas}</span></div>
                        <div className="sum-row"><span>Alquileres</span><span style={{ fontWeight: 500 }}>{alq}</span></div>
      ...
      {tab !== 'DASHBOARD' && (
        <>
          <div className="toolbar">
            ...
            <select value={firmoFilt} onChange={e => setFirmoFilt(e.target.value as Firmo)}>
                  ...
                  <tbody>
                    {tabData.length === 0 ? (
                      ...
                      <tr key={r.id}>
                        ...
                        <td>
                          <span className={`badge ${r.operacion === 'VENTA' ? 'badge-red' : 'badge-blue'}`}>
                            {r.operacion}
                          </span>
                        </td>
                        ...
                        <td>
                          <span className={`badge ${r.firmo === 'FIRMADO' ? 'badge-green' : 'badge-amber'}`}>
                            {r.firmo}
                          </span>
                        </td>
                        <td>
      ...
      {modal === 'new' && (
        <ReservaModal
          tab={tab as 'EMPRENDIMIENTOS'|'RESIDENCIAL'|'COMERCIAL'}
      ...
      {modal === 'edit' && sel && (
        <ReservaModal
          ...
          onSaved={() => { setModal(null); setSel(null); load() }}
        />
      )}
    </div>
  )
...
function ReservaModal({ tab, reserva, onClose, onSaved }: {
  ...
  // Get unidades for this tab
  const unidades = Object.entries(UNIDADES_BY_TAB).find(([k]) => k === tab)?.[1] || []
  ...
  async function save() {
    ...
    try {
      const payload = {
        ...
        operacion: op, unidad: unidad || tab,
        monto_ars: arsV ? parseFloat(arsV) : null,
        monto_usd: usdV ? parseFloat(usdV) : null,
        modo_pago: pago, firmo,
      }
      if (isEdit && reserva) {
        const { error } = await supabase.from('reservas').update(payload).eq('id', reserva.id)
  ...
  return (
    <Modal
      title={isEdit ? `Editar reserva — ${reserva?.direccion?.slice(0,30)}` : `Nueva reserva — ${tab.charAt(0)+tab.slice(1).toLowerCase()}`}
      onClose={onClose}
      footer={<>
    ...
    >
      <div className="form-grid">
        ...
        <FG label="Broker">
          <input placeholder="Nombre del broker" value={broker} onChange={e => setBroker(e.target.value)} />
        </FG>
        <FG label="Cliente">
