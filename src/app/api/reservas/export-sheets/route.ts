...
export async function POST(req: NextRequest) {
  ...
  // Buscar en Drive por nombre fijo
  ...
  // Si el archivo existía, limpiar las 4 pestañas primero
  if (existingFile) {
    // Verificar que las pestañas existen; si no, crearlas
    ...
    const wantedSheets = [
      { title: 'Reservas', sheetId: 0 },
      { title: 'Emprendimientos', sheetId: 1 },
      { title: 'Residencial', sheetId: 2 },
      { title: 'Comercial', sheetId: 3 },
    ]
    const addRequests = wantedSheets
      .filter(w => !existingSheets.some(e => e.title === w.title))
      .map(w => ({ addSheet: { properties: { title: w.title, sheetId: w.sheetId } } }))

    // Clear ranges
    const clearRanges = ['Reservas!A:Z','Emprendimientos!A:Z','Residencial!A:Z','Comercial!A:Z']
    const clearBody = { ranges: clearRanges }

    const [, clearRes] = await Promise.all([
      addRequests.length
        ? fetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}:batchUpdate`, { method:'POST', headers:gHeaders, body:JSON.stringify({ requests: addRequests }) })
        : Promise.resolve(),
      fetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values:batchClear`, { method:'POST', headers:gHeaders, body:JSON.stringify(clearBody) }),
    ])
    if (!clearRes.ok) {
      const err = await clearRes.json()
      return NextResponse.json({ error: 'clear_failed', detail: err }, { status: 500 })
    }
  }
